function [finalDecisions, finalP, finalX, finalProfit, finalDemands, finalUtilities, finalGhg, finalCost, betas] = evolutionary(a,ghgLim,isBev,n,draws)
...
while turn < 2 % abs(change) > 0.1 & turn < 30
    for f = 1:n.F
        turn = turn+1;
        compIdx = 1:n.F*n.J;
        lowIdx = 1+n.J*(f-1);
...
finalDecisions = xResultsScaled(end,:);
finalProfit = profitResults(end);
